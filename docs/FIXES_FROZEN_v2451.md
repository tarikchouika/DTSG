...
|---|---|---|
| 1 | «إنسى أمر ڤيرسيل… نستخدمها إحتياطياً من أجل التجريب» | **Cloudflare** (Pages + Workers + R2) هي مسار النشر. `vercel.json` و`.vercelignore` **مستثناة في `.gitignore`** ولا تُرفع. **لا تُحذف** `Dockerfile`/`.dockerignore` من أجل ڤيرسيل. |
...
| # | الإصلاح | الحارس |
|---|---|---|
| 15 | لا عنصر رهان في شاشة اللعب: أُزيلت الكتلة الميتة `#chessStake` من `chessUpdateHUD()`؛ صفّ الرهان `#chessBet` **مخفي** في الإعداد؛ `#chessHotSeat`/التدريب لا يخصمان من الرصيد | `tests/_chess_browser_test.js` (الجزء 1 §8 + الجزء 2) |
