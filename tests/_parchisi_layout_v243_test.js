/* ═══════════════════════════════════════════════════════════════════════════
   ...
   5) القواعد الملوّنة صغيرة (160) والخانات المستطيلة مكبَّرة (93.33×30)
...
(async () => {
  ...
  for (const [label, vp] of VIEWPORTS) {
    ...
    const g = m.geom;
    (g.B === 160 && Math.abs(g.ARM - 93.33) < 0.02 && Math.abs(g.PITCH - 30) < 0.01 && g.CTR === 120)
      ? ok('القاعدة 160 · الخانة ' + g.ARM + '×' + g.PITCH + ' · المركز ' + g.CTR)
      : bad('الهندسة: ' + JSON.stringify(g));
    (Math.abs(g.R - 12.1) < 0.05 && Math.abs(g.RHOME - 13.8) < 0.05)
      ? ok('البيادق مكبَّرة +15% (' + g.R + ' · عش ' + g.RHOME + ')')
      : bad('أنصاف أقطار البيادق: ' + JSON.stringify(g));

    errs.length === 0 ? ok('بلا أخطاء كونسول') : bad('أخطاء: ' + errs.slice(0, 3).join(' | '));
    await page.screenshot({ path: '/tmp/pr-243-' + (vp.width > vp.height ? 'land' : 'port') + '.png' });
    await ctx.close();
  }
  await browser.close();
