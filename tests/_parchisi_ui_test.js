...
(async () => {
  ...
  const sizes = await p.evaluate(() => {
    ...
    /* (1) [v2.44] بيدقان على خانة عادية → نصف القطر 11.2 ويستوعبهما الممر (27.5) */
  ...
  /* [v2.44] الخانة كلاسيكية 64×27.5 ⇒ البيدق في الممر 11.2 (قطر 22.4) والخطوة 24.2 */
  ok('بيدقان: نصف القطر ثابت 11.2', Math.abs(pr[0].r - 11.2) < 0.05 && Math.abs(pr[1].r - 11.2) < 0.05);
  ok('بيدقان: بلا تغطية (الخطوة ≥ القطر)', dPair >= 22.4);
  const tr = sizes.triple;
  /* [v2.44] ثلاثة في الممر: قطر مصغَّر ملائم (10.6) وبلا تغطية */
  ok('ثلاثة (تصغير الممر مسموح): قطر ' + (tr[0].r) + ' وبلا تغطية',
     tr.every(q => Math.abs(q.r - 10.6) < 0.05) &&
     Math.hypot(tr[0].x - tr[1].x, tr[0].y - tr[1].y) >= 21.2);
