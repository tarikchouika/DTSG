...
(async () => {
  ...
  try {
    ...
    /* default difficulty = expert (mult 3.0 منذ [AI-MAX]), human = White. Start. */
    await page.click('#damaGo');
    await wait(page, () => { const bd = document.getElementById('damaBoard'); return bd && bd.children.length === 64; });
    await sleep(200);

    const before = await page.evaluate(() => ST.gold);   /* تدريب مجاني: بلا خصم */
    R.push(['training is free: no bet taken (50000)', before === 50000]);

    /* ── WIN path: فوز تدريبي بلا إضافة رصيد ── */
    ...
    await page.evaluate(() => damaNewMatch());
    await sleep(150);
    const g0 = await page.evaluate(() => ST.gold);   /* 50000 */
    ...
    R.push(['draw keeps gold (50000)', drawState.gold === 50000]);
