...
(function () {
  ...
  var ob = eng.obligationPiece(s);
  ok('الضائم هو المُلزَم قبل البيدق العادي', ob && ob[0] === 6 && ob[1] === 1);
  /* أكل بالبيدق متجاهلاً الملك ⇒ يُنفخ الملك */
  var pawnCap = eng.legalMoves(s, WHITE).find(function (m) { return m.cap && m.from[0] === 6 && m.from[1] === 5; });
  var info = eng.applyMove(s, pawnCap);
  ok('تجاهل الضائم ⇒ يُنفخ الضائم', !!info.souffled && info.souffled[0] === 6 && info.souffled[1] === 1);

  /* ب) السلسلة الأطول أُلزم: بيدق 2-سلسلة قبل بيدق 1-سلسلة */
  var s2 = stateOf([
    dot, dot, '...b....', dot,
    '.b...b..', 'w...w...', dot, dot
  ], WHITE);
  var ob2 = eng.obligationPiece(s2);
  ok('صاحب السلسلة الأطول هو المُلزَم', ob2 && ob2[0] === 5 && ob2[1] === 0);
  /* الأكل بالبيدق الأقصر ⇒ يُنفخ صاحب السلسلة الأطول */
  var shortCap = eng.legalMoves(s2, WHITE).find(function (m) { return m.cap && m.from[0] === 5 && m.from[1] === 4; });
  var info2 = eng.applyMove(s2, shortCap);
  ok('الأكل بسلسلة أقصر من المتاحة للمُلزَم ⇒ نفخ المُلزَم', !!info2.souffled && info2.souffled[0] === 5 && info2.souffled[1] === 0);

  /* ج) المُلزَم أكل لكن بسلسلة أقصر من أقصى سلاسله ⇒ يُنفخ لعدم الإتمام */
  var s3 = stateOf([
    dot, dot, '.b......', dot,
    '.b.b....', '..w.....', dot, dot
  ], WHITE);
  var ob3 = eng.obligationPiece(s3);
  ok('المُلزَم (5,2) بسلسلته الثنائية', ob3 && ob3[0] === 5 && ob3[1] === 2);
  ok('أقصى سلسلة مطلوبة = 2', eng.maxChainAt(s3.grid, 5, 2) === 2);
  var oneHop = eng.legalMoves(s3, WHITE).find(function (m) { return m.cap && m.from[0] === 5 && m.from[1] === 2 && m.to[0] === 3 && m.to[1] === 4; });
  ok('القفزة الأحادية متاحة (حرية اللاعب)', !!oneHop);
  var info3 = eng.applyMove(s3, oneHop);
  /* القطعة المُلزَمة نُفخت في موضعها بعد القفزة (3,4) */
  ok('أكمل قفزة واحدة بلا تتمة ⇒ يُنفخ لعدم إتمام السلسلة', !!info3.souffled && info3.souffled[0] === 3 && info3.souffled[1] === 4 && s3.grid[3][4] === null);

  /* د) إتمام السلسلة الكاملة ⇒ لا نفخ */
  var s4 = stateOf([
    dot, dot, '.b......', dot,
    '.b.b....', '..w.....', dot, dot
  ], WHITE);
  var twoHop = eng.legalMoves(s4, WHITE).find(function (m) { return m.cap && m.from[0] === 5 && m.from[1] === 2 && m.to[0] === 3 && m.to[1] === 0; });
  var i4 = eng.applyMove(s4, twoHop);
  ok('التتمة معروضة بعد القفزة الأولى', i4.continued === true);
  var cont4 = eng.legalMoves(s4, WHITE);
  var i5 = eng.applyMove(s4, cont4[0]);
  ok('إتمام السلسلة الثنائية ⇒ لا نفخ ودور الخصم', i5.souffled === null && s4.turn === BLACK);
})();
