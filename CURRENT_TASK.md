...
## ملاحظة توثيقية — تقرير إعادة التحقق الأمني (23 شتنبر 2026، وثيقة المستخدم)
تقرير المالك (POST-FIX SECURITY REVIEW) يوثّق 19 بنداً: 13 مغلقاً، 3 جزئياً، 2 مفتوحان **خارج نطاق جلسة اليوم**:
- **DTSG-008** (HIGH): ترويسة `x-backend-addr` ما زالت تكشف عنوان نفق الهاتف — مفتوح.
- **DTSG-011** (HIGH): كوكي الجلسة `SameSite=None` + ترويسة Set-Cookie مكرّرة عند الدخول — مفتوح (NEW-5 هو الوجه الإطلاعي لها).
...
## جولة-4 — تقرير التحقق (2026-09-23) — الرد: v2.59 في `main` ✅

> **الخلاصة**: التقرير حكم CONDITIONAL — 19 بنداً أُغلقت وتحقق منها مستقلاً
> (تشغيلهم المستقل: sec-audit 28/28 + ops-guards 38/38 مطابقتين لنا)، وبقيت
> عائمتان داخل عائلة واحدة + ملاحظتان. **كل ما كان مطلوباً كودياً أُنجز في
> v2.59 ودُفع مباشرة في `main`** — المتبقي كله تنفيذ على الهاتف/الحسابات.

### ما أُغلق كودياً (v2.59 — هذا المستودع)

| بند | الحالة | أين |
|---|---|---|
| **R3-001 (حرجة)** `/api/bot/request` بلا مصادقة (سحب شبحي + خصم فوري) | ✅ سِرّ مشترك `x-bot-secret` (BOT_API_SECRET/ADMIN_API_SECRET) + مقارنة ثابتة الزمن + 401 fail-closed + 30/د/IP | `server-payments.js` |
| **R3-002 (عالية)** `/api/bot/link` بلا مصادقة (ربط تيليغرام شبحي) | ✅ نفس البوابة | `server-payments.js` |
| **R4-001 (متوسطة)** CORS يثق بـ`.e2b.app`/`.arena.ai` (ساندبوكس عامة) | ✅ خلف `DM_TEST_MODE=1` أو `DM_DEV_ORIGINS` صريحة — الإنتاج 403 | `server.js` + `server-payments.js` |
| **R4-003** `DM_TEST_MODE` يجمع 3 سلوكيات خطرة | ✅ حارس إقلاع: تحذير صاخب دائم مع علامتي إنتاج فأكثر | `server.js` |
| البوت الشرعي لا يكسر | ✅ `scripts/voucher-bot.js` يرسل الترويسة تلقائياً + `verify-phone-v244.sh` مُحدَّث (فحص 401 جديد) | `scripts/` |
| طبقة دفاع ثانية في الوسيط | ✅ `admin-act` يقبل ترويسة السرّ أيضاً (لو ضُبط `BOT_API_SECRET` في بيئة CF) | `cf-worker/payments-core.js` |

الحراس: **ops-guards 48/48** (كان 38) · **sec-audit 32/32** (كان 28) — البنية
الباقية كلها خضراء (fin-logs 22/22 · coin+tourney 34/34 · admin-i18n 53/53 ·
الإحصائية 47+5+4/0). ملاحظة: اختبارات الرامي بالمتصفح لم تُعد هذه الجولة —
لم يمس v2.59 أي واجهة/لعبة (خادم + سكربتات + اختبارات فقط)، والبيئة
(Playwright) أُعيد بناؤها عند الحاجة.

### ما تبقّى — تسلسل الإطلاق (تقرير §7، بالترتيب)
...
- كوكي `SameSite=None` (مخفّف ببوابة الأصل؛ الإصلاح الكامل مع نقل الـAPI إلى
  `api.dtsg.pages.dev` — متوسط المدى).
- سقف استضافة الهاتف الواحد (~30 توازياً + انقطاعات النفق — جولة-2/3).
- «تأكيد من طرف المنصة» لربط البوت (توصية التقرير §6-3): المسار الحالي آمن
  بالبوابة (البوت وحده يملك السر)؛ يُدرج في الطريق إذا رغب المالك.
